EM <-function (y, X = NULL, ETA = NULL, R = NULL, init = NULL, iters = 50, 
          REML = TRUE, draw = TRUE, silent = FALSE, forced = NULL) 
{
  nanana <- names(ETA)
  ...
  if (is.null(X) & is.null(ETA)) {
    ...
    ETA <- lapply(ETA, function(x) {
      if (length(x) == 1) {
        x[[2]] <- as(diag(dim(x[[1]])[2]), Class = "sparseMatrix")
      }
      else {
        x <- x
      }
    ...
    if (is.null(forced)) {
      while (conv == 0) {
        ...
        CInv <- try(solve(CC), silent = TRUE)
        PROBLEM <- FALSE
        if (class(CInv) == "try-error") {
          vava <- as.list(lapply(var.com.sto, function(x, 
                                                       y) {
            x[y]
          }, y = wi - 1))
          axs = lapply(vava, function(x) {
            varE/x
          })
          CC = numeric()
          for (j in 1:length(ETA)) {
        ...
        for (k in 1:(length(var.com) - 1)) {
          ...
          var.com[[k]] = ((t(thetaHat[pairs.a[[rrr]], 
                                      ]) %*% Kinv %*% thetaHat[pairs.a[[rrr]], 
                                                               ]) + matrix.trace(Kinv %*% CInv[pairs.a[[rrr]], 
                                                                                               pairs.a[[rrr]]] * as.numeric(varE)))/nrow(ETA[[rrr]][[2]])
        }
        ...
        if (draw) {
          ...
          plot(var.com.sto[[1]], ylim = c(0, ylim), type = "l", 
               col = my.palette[1], lwd = 3, xlim = c(0, 
                                                      iters), xaxt = "n", las = 2, main = "Expectation-Maximization algorithm results", 
               ylab = "Value of the variance component", 
               xlab = "Iteration to be processed to reach convergence", 
               cex.axis = 0.8)
        ...
        if (wi > 1) {
          ...
          uuu2 <- apply(uuu1, 1, function(x) {
            if (abs(x[1] - x[2]) < x[1]/100) {
              y <- TRUE
            }
            else {
              y <- FALSE
        ...
        if (change == length(var.com) | wi == iters) {
          conv = 1
          if (!silent) {
          ...
          if (wi == iters) {
            if (!silent) {
              ...
              my.palette <- brewer.pal(7, "Accent")
              layout(matrix(1, 1, 1))
              plot(var.com.sto[[1]], ylim = c(0, ylim), 
                   type = "l", col = my.palette[1], lwd = 3, 
                   xlim = c(0, iters), xaxt = "n", las = 2, 
                   main = "Expectation-Maximization algorithm results", 
                   ylab = "Value of the variance component", 
                   xlab = "Iteration to be processed to reach convergence", 
              ...
              for (k in 1:length(var.com)) {
                if (k == length(var.com)) {
                  ...
                  lege[[k]] <- paste("Var(", varosss[k], 
                                     "):", round(var.com.sto[[k]][ww], 
                                                 4), sep = "")
                }
              }
              legend("topleft", bty = "n", col = my.palette, 
                     lty = 1, legend = unlist(lege), cex = 0.75)
    ...
    if (REML == TRUE) {
      logL = as.numeric(-0.5 * ((determinant(V, logarithm = TRUE)$modulus[[1]]) + 
                                  determinant(xvx, logarithm = TRUE)$modulus[[1]] + 
                                  t(y) %*% (pm %*% y)))
    }
    else {
      logL = as.numeric(-0.5 * ((determinant(V, logarithm = TRUE)$modulus[[1]]) + 
    ...
    if (PROBLEM) {
      ...
      cat(paste("\nCoefficient matrix couldn't be inverted anymore after", 
                wi, "iterations\nYou might have overspecified the model with random effects with zero variance\nMLE's for variance components returned. Check plot of last progress.\n"))
    }
    VCC <- (as.matrix(out1, ncol = 1))
    rownames(VCC) <- c(paste("Var(", varosss, ")", sep = ""), 
                       "Var(Error)")
    output <- list(var.comp = VCC, V.inv = Vinv, u.hat = u, 
