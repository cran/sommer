    ...
    if(is.null(forced)){ # &&&&&&&&&&&&&&&&& IF NOT FORCED &&&&&&&&&&&&&&&&&&&&&
      ...
      while (conv==0) { ## START!!!!! ===============================
        ...
        for(j in 1:length(ETA)){ # for each variance component
          ...
          for(k in 1:length(ETA)){ #multiply it for all other variance components
            if(j == k & names(ETA[[j]])[1] != "X"){ ## diagonal element of CC and not fixed
              ...
              solvedK <- try(solve(as.matrix(ETA[[j]][[2]])), silent = TRUE)
              if (class(solvedK) == "try-error") {
                #print("error")
                diag(ETA[[j]][[2]]) <- diag(ETA[[j]][[2]]) + (rep(1e-06, dim(ETA[[j]][[2]])[2]))
        ...
        if (class(CInv) == "try-error") {
          vava <- as.list(lapply(var.com.sto,function(x,y){x[y]},y=wi-1))# as.list(var.com.sto[[wi-1]]) #lapply(axs, function(x){round(x,3)})#axs*rnorm(length(axs),0,.05)
          axs = lapply(vava, function(x){varE/x})
          #print(axs)
          CC = numeric()
          ################### ROWS OF MME
          for(j in 1:length(ETA)){ # for each variance component
          ...
          ## second try fail then finish with those var comp and return a warning message
          CInv<-solve(CC) 
          conv=1
          PROBLEM=TRUE
          draw=TRUE
