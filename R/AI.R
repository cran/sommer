          ...
          gss = TRUE, forced = NULL) 
{
  ...
  if (is.null(X) & is.null(ZETA)) {
    ...
    y2 <- y
    good <- which(!is.na(y))
    if (length(ZETA) == 1 & EIGEND == TRUE & (dim(ZETA[[1]][[1]])[2] == 
                                              dim(ZETA[[1]][[2]])[2])) {
    ...
    if (length(ZETA) == 1 & EIGEND == TRUE & (dim(ZETA[[1]][[1]])[1] == 
                                              dim(ZETA[[1]][[2]])[2])) {
      if (is.null(init)) {
        var.com <- c(rep(1e-04, nvarcom))
      }
      else {
        var.com <- init/var.y
      }
    ...
    nmbb <- TRUE
    dimen.zeta <- unlist(lapply(ZETA, function(x) {
      lapply(x, function(y) {
        d1 <- dim(y)
        re <- 0
        if (d1[2] > d1[1]) {
    ...
    if (is.null(forced)) {
      while (conv == 0) {
        ...
        if (wi > 10 & gss == TRUE) {
          joke <- round(dim(ups)[2] * 0.2)
          todo <- (dim(ups)[2] - joke):dim(ups)[2]
          pushes <- apply(ups[, todo], 1, function(fg) {
            length(which(fg < 0))/length(fg)
          })
          kkkkk <- length(which(pushes > 0.4))
          if ((zig.zag(logL2.stored[(length(logL2.stored) - 
                                     4):length(logL2.stored)]) == 1) & (kkkkk == 
                                                                        0)) {
            wi = iters
            if (!silent) {
          ...
          else if ((zig.zag(logL2.stored[(length(logL2.stored) - 
                                          4):length(logL2.stored)]) == 1) & (kkkkk != 
                                                                             0)) {
            wi = iters
          }
  ...
  if (length(ZETA) == 1 & EIGEND == TRUE & (dim(ZETA[[1]][[1]])[2] == 
    ...
    for (k in zvar) {
      Uxi <- solve(t(Us[[k]]))
      u[[k]] <- Uxi %*% (((ZETA3[[k]][[2]] * as.numeric(var.com2[k, 
                                                                 1])) %*% t(ZETA3[[k]][[1]]) %*% Vinv %*% ee))
      Var.u[[k]] <- (as.numeric(var.com2[k, 1])^2) * Uxi %*% 
        tcrossprod(crossprod(ZETA3[[k]][[1]] %*% ZETA3[[k]][[2]], 
                             pm) %*% (ZETA3[[k]][[1]] %*% ZETA3[[k]][[2]]), 
                   Uxi)
      PEV.u[[k]] <- as.numeric(var.com2[k, 1]) * ZETA3[[k]][[2]] - 
        Var.u[[k]]
    }
  }
  else {
    for (k in zvar) {
      u[[k]] <- ((ZETA3[[k]][[2]] * as.numeric(var.com2[k, 
                                                        1])) %*% t(ZETA3[[k]][[1]]) %*% Vinv %*% ee)
      Var.u[[k]] <- (as.numeric(var.com2[k, 1])^2) * (crossprod(ZETA3[[k]][[1]] %*% 
                                                                  ZETA3[[k]][[2]], pm) %*% (ZETA3[[k]][[1]] %*% 
                                                                                              ZETA3[[k]][[2]]))
      PEV.u[[k]] <- as.numeric(var.com2[k, 1]) * ZETA3[[k]][[2]] - 
        Var.u[[k]]
    }
  ...
  if (!abnormal & !abnormalVE) {
    cat("\nError variance was pushing to be negative. Be careful, model might be overspecified.\n")
  }
  if (EIGEND) {
    Usi <- solve(t(Usp))
    ee <- Usi %*% ee
    residuals3 <- (Usi %*% y) - fitted.y[good]
    Vinv <- Usi %*% tcrossprod(Vinv, Usi)
  }
