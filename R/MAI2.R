MAI2 <- function(Y,X=NULL,ZETA=NULL,init=NULL,maxcyc=20,tol=1e-3,draw=TRUE,silent=FALSE,constraint=FALSE,EIGEND=FALSE,forced=NULL){
  ...
  #init <- MAIhelp(Y,X=X,ZETA=ZETA,init=init,maxcyc=3,tol=tol,draw=draw)$var.comp
  ...
  X <- do.call("adiag1", rep(list(as.matrix(X)), ts)); dim(X)
  tX <- t(X); dim(tX)
  qr <- qr(X)
  ...
  if(is.null(init)){ # var.com <- rep(list(sc.var/(nvarcom+6)),nvarcom+1).../(nvarcom+2)
    (sigma <- rep(list(sc.var/(nvarcom+2)),nvarcom+1)) #original variance values
  }else{
    sigma <- init
  }
  ...
  for(cycle in 1:maxcyc){
    ...
    for(i in 1:(nvarcom+1)){
      if(i<=nvarcom){ # ZETA[[i]]$Z %*% tcrossprod(ZETA[[i]]$K, ZETA[[i]]$Z)
        ...
        #listGs[[i]] <- kronecker(as.matrix(listKs[[i]]),sigma[[i]]) #MNR
        if(EIGEND){
          listGs[[i]] <- as(kronecker(sigma[[i]],as.matrix(listKs[[i]])),Class="sparseMatrix")
        }else{
    ...
    ytPy <- t(Y)%*%(P%*%Y)
    llik=as.numeric(-0.5*((ddv)+determinant(xvx, logarithm = TRUE)$modulus[[1]]+ytPy)) # log likelihood, problem
    #
    ...
    for(v in 1:(nvarcom+1)){ ## FOR EACH VAR.COMP
      ...
      #deriv.list.vc[[v]] <- lapply(posmats.list.vc[[v]],function(x,y){P %*% as(kronecker(y,x),Class="sparseMatrix")},y=as.matrix(deriva))# MNR deriva=ZKZ
    ...
    x <- sapply(TT,function(x) 0.5 * (as.numeric(t(Y) %*% x %*% P %*% Y - sum(diag(x)))) )
    ...
    ## Average Information y'ViPVjPPy .... A*=Vi=dV/ds .... [Vi Vj'] si sj
    # 0.5 * (tY %*% at[[h1]] %*% P %*% at[[h2]] %*% crossprod(P, Py))[1]
    Py <- crossprod(P,Y)
    ff <- function(x) 0.5 * as.numeric( ( t(Y)%*%TT[[x[1]]]%*% t(TT[[x[2]]])%*%Py) )  
    aa <- apply(entries,1,ff) # matrix of combinations of var.comp 
    A[as.matrix(entries)] <- aa
    #print(aa)
    #print(entries)
    #print(A)
    AI.svd <- solve(A) # Inverse of Fishers
    ...
    if(cycle ==1){ #controling illegal values
      bad <- which(abs(up)  > 1)
      #if(length(bad)>0){up[bad,]<-0}
    }else{
      bad2 <- which(abs(up)  > 1)
    }
    ups <-cbind(ups,up)
    coef2 <- coef2 + (taper[cycle]*up) # sigma + f[s*F-*dL/ds] ..... = coef + taper[x]
    ...
    #print(bad)
    #print(bad2)
    if(length(bad)>0){sigma2[bad,] <- .00001;bad<-NULL}
    if(length(bad2)>0){sigma2[bad2,] <- 0}
    ...
    #print(sigma)
    ### only needed in AI not in NR because AI handles bad the zero var.comp
    sigma <- lapply(sigma,function(x){bad <- which(diag(x)<0); if(length(bad)>0){diag(x)[bad] <- 0.0001};return(x)})
    #sigma[[1]][1,1:2]<-0
    #sigma[[1]][1:2,1]<-0
  ...
  if(constraint){
    ...
    if(length(where) >0){
      ...
      for(cycle in 1:maxcyc){
        ...
        #print(sigma)
        #sigma <- lapply(sigma,function(x){bad <- which(diag(x)<0); if(length(bad)>0){diag(x)[bad] <- 0.001};return(x)})
        #sigma[[1]][1,1:2]<-0
        #sigma[[1]][1:2,1]<-0
  ...
  vmi <- try(solve(as(W, Class="sparseMatrix")), silent = TRUE)
  W <- NULL
  if(!silent){
    setTxtProgressBar(pb, (tot/tot))### keep filling the progress bar
  }
  ...
  for (k in 1:nvarcom) { # GZ'V-(y-Xb)
    ...
    #provi <- Uxi %*% ( ( (ZETA3[[k]][[2]]*as.numeric(var.com2[k,1])) %*% t(ZETA3[[k]][[1]]) %*% Vinv %*% ee )) # u.hat = GZ'V-(y-Xb)
