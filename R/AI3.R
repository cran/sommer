AI3 <- function(y, X=NULL, ZETA=NULL, R=NULL, draw=TRUE, REML=TRUE, silent=FALSE, init=NULL, iters=50, forced=NULL, sherman=FALSE){
  y.or <- y
  ...
  if(is.null(X) & is.null(ZETA)){ #
    ...
    x.or <- as.matrix(xm)
    zeta.or <- ZETA
    zeta.or  <- lapply(zeta.or , function(x){lapply(x, as.matrix)}) 
    ZETA2 <- ZETA; y2 <- y ; good <- which(!is.na(y)) 
    ...
    while (conv==0) {
      ...
      if(REML==TRUE){ 
        ...
        if(is.infinite(ddv)){ 
          ...
          logL=as.numeric(-0.5*((ddv) + t(yv)%*%pm%*%yv )) 
        }
      }
      ...
      if (abs(logL-logL2)<0.001 | wi == iters ){ ## 
        ...
        dldv=matrix(0,nvarcom) # m
        ##########
        for(k in 1:nvarcom){
          prm=pm%*%om[[k]] # PHi# ( P(e) = Vinv - [Vinv X (X'V-X)- X Vinv]  )  %*% I or K, etc       from I 
          
          tr1=sum(diag(prm))
          # fill the first derivative matrix
          dldv[k,1]=-0.5*tr1+0.5*as.numeric(t(yv)%*%prm%*%py) # -.5 * [ tr(PHi)+0.5 * y' %*% PHi %*% Py ]        ## 
        }
