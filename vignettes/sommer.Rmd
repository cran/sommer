...
The sommer package was developed to provide R users with a powerful multivariate mixed model solver for different genetic and non-genetic analysis in diploid and polyploid organisms. This package allows the user to estimate variance components for a mixed model with the advantage of specifying the variance-covariance structure of the random effects and obtain other parameters such as BLUPs, BLUEs, residuals, fitted values, variances for fixed and random effects, etc. 

The package is focused on genomic prediction (hybrid prediction & genomic selection) and GWAS analysis, although general mixed models can be fitted as well. The package provides kernels to estimate additive (`A.mat`), dominance (`D.mat`), and epistatic (`E.mat`) relationship matrices that have been shown to increase prediction accuracy under certain scenarios. The package provides flexibility to fit other genetic models such as full and half diallel models as well.
...
2) Specifying heterogeneous variances in univariate mixed models
3) Using the pin calculator
4) Half and full diallel designs
5) Genome wide association analysis (GWAS) in diploids and tetraploids
6) Genomic selection
7) Single cross prediction
...
The core of the package are the `mmer` (matrix-based) and `mmer2` (formula-based) functions which solve the mixed model equations. The functions are an interface to call one of the 4 ML/REML methods supported in the package; `EMMA` efficient mixed model association (Kang et al. 2008), `AI` Direct-Inversion Average Information (Gilmour et al. 1995; Lee et al. 2015), `EM` expectation maximization (Searle 1993; Bernardo 2010), and the default `NR` Direct-Inversion Newton-Raphson (Tunnicliffe 1989). All methods can handle multiple random effects and covariance structures and multivariate (multiple responses) mixed models. 
...
Notice that we used the random effect `g(id)` to adjust for the population structure whereas the markers are added in the `W` argument. Internally there's 2 option for this 2-step GWAS models, in the default `P3D=TRUE` the model estimate variance components once and test each marker by a generalized least square process adjusting by the inverse of the V matrix (ZKZ'+R). If the user sets the argument `P3D=FALSE` the model will estimate variance components for each marker tested by adding the marker being tested as a fixed effect in the model so the variance components are adjusted. This can be quite time consuming and the first option presented usually yields very good results. For more details go to the help page of the `mmer` or `mmer2` functions. 
...
Notice the use of the `models` argument. The default is the additive model which applies for diploids and polyploids but for polyploids the model can be a vector with all possible models, i.e. "additive","1-dom-alt","1-dom-ref","2-dom-alt","2-dom-ref" models are supported for polyploids based on Rosyara (2016)
...
Notice that here we made use of several arguments that were not neccesarily.
