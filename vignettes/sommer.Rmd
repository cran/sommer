...
## 6) Multivariate gentic models

Sometimes is important to estimate genetic variance-covariance among traits, multi-reponse models are very useful for such task. Currently sommer only deals with multivariate models for a single random effect of the form:
...
We can specify the K matrices. The main difference between this model and the rest is that we will get covariance among traits related to the random effect specified and covariance among error variances. BLUPs will also be corrected for such covariances usually leading to more accurate predictions.
...
Now you can extract the BLUPs using the 'randef' function or simple accesing with the '$' sign and pick 'u.hat'. Calculate genetic correlations and heritabilities easily. Good luck with your analysis.
